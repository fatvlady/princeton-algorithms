...
public class BoggleTrieSET implements Iterable<String>  {
    private static final int R = 26;        // uppercase ASCII
    ...
    BoggleTrieSET.Node get(BoggleTrieSET.Node x, String key, int d) {
    ...
    BoggleTrieSET.Node get(BoggleTrieSET.Node x, char c) {
        if (x == null) return null;
        return x.next[c - 'A'];
    }

    BoggleTrieSET.Node getRoot() {
        return root;
    }

    static boolean isTerminalNode(BoggleTrieSET.Node x) {
    ...
    void collect(BoggleTrieSET.Node x, StringBuilder prefix, Queue<String> results) {
        if (x == null) return;
    ...
    void collect(BoggleTrieSET.Node x, StringBuilder prefix, String pattern,
    ...
    int longestPrefixOf(BoggleTrieSET.Node x, String query, int d, int length) {
    ...
    BoggleTrieSET.Node delete(BoggleTrieSET.Node x, String key, int d) {
        if (x == null) return null;
        if (d == key.length()) {
            if (x.isString) n--;
            x.isString = false;
